...
/**
 * The single authorization rule for a route/round. The owner sees and acts
 * on every route; a driver may only touch a round that is unassigned
 * (claimable) or already theirs. Enforce this on every route read and
 * mutation a driver can reach, not just the plan endpoint.
 */
export function assertRouteAccessible(route: { driverId: string | null }, actor: RouteActor) {
  if (actor.role === "OWNER") return;
  if (route.driverId && route.driverId !== actor.userId) {
    throw new ForbiddenError("This round belongs to another driver. Ask the owner to assign your round.");
  }
}
...
export async function reoptimizeRoute(
  ...
  assertRouteAccessible(current, actor);
  ...
  const newRoute = await db.$transaction(async (tx) => {
    ...
    const created = await tx.route.create({
      data: {
        ...
        driverId: claimedDriverId(current.driverId, actor),
...
export async function getActiveRouteForDate(dateString: string) {
  return db.route.findFirst({
    ...
    // revisionNumber picks the latest revision of one round; createdAt breaks
    // ties deterministically when several independent rounds share the date.
